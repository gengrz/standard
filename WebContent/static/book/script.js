...
//	getjjrszModel();
	...
	layui.use(['layer'], function(){
	    ...
	    $("#appointment_btn").click(function(){
			//监听指定开关
		 	var appointment_state = $("input[type='checkbox']").is(':checked') ? 1 : 0;
			var dt = $("#hidden_date").val();
			var am_count = $("#am_count").val();
			var pm_count = $("#pm_count").val();
			var am_id = $("#am_id").val();
			var pm_id = $("#pm_id").val();
//			layer.msg("data");
			$.post(ctx + '/book/appointmentCalendar/saveDate',{appointment_state:appointment_state,appointment_date:dt,am_count:am_count,pm_count:pm_count,am_id:am_id,pm_id:pm_id},function(data){
				setRigth(new Date().getFullYear(),new Date().getMonth()+1,new Date().getDate());
//				getjjrszModel();
				if(data){
					layer.msg("保存成功！");
	    ...
	    //设置周末不可预约按钮事件
	    $("#setWeeked").click(function(){
	    	$.post(ctx + '/book/appointmentCalendar/setWeekend',function(data){
				setRigth(new Date().getFullYear(),new Date().getMonth()+1,new Date().getDate());
//				getjjrszModel();
				if(data){
					layer.msg("保存成功！");
...
function getjjrszModelByYear(year){
	jjrmodelidlist=['1','2','2'];
	jjrmodeltimelist=['2015-08-10 00:00:00','2015-08-11 00:00:00','2015-08-12 00:00:00']; //这里时间的格式为yyyy-MM-dd HH:mm:ss
	jjrmodelztlist=['1','2','2'];  //1为上班，2为休息
//	$.ajax({
//		type:"POST",
//		url:ctx + "/book/",
//		async:false,
...
//function getjjrszModel(){
////	jjrmodelidlist=['1','2','2'];
////	jjrmodeltimelist=['2018-08-10 00:00:00','2018-08-11 00:00:00','2018-08-12 00:00:00']; //这里时间的格式为yyyy-MM-dd HH:mm:ss
////	jjrmodelztlist=['1','2','2'];  //1为上班，2为休息
//	$.post(ctx + "/book/appointmentCalendar/getWeekend",function(json){
//		var data=json.data;
//		for(var i=0;i<data.length;i++){
//			jjrmodelidlist.push(data[i].jjr_id);
//			jjrmodeltimelist.push(data[i].jjr_time);
//			jjrmodelztlist.push(data[i].jjr_zt);
//			console.log(jjrmodelztlist)
//			console.log(jjrmodeltimelist)
//			console.log(jjrmodelidlist)
//		}
//	})
//}
...
//	currday.style.lineHeight="80px";    //实际在得到长宽时不能用style.height，得用.offsetHeight,但是设置的时候要用style.height=...
//	var szrq=withClass("aboluo-ssjjr");
//	szrq.style.marginTop="20px";
//	var span2=newElement('span');
//	span2.innerHTML="设置预约人数:";
...
//	szrqselect.className="layui-input";
...
//	szrq.appendChild(szrqbutton);
	...
	var date=setdateinfo(year,yue,day);
	var nowDate = formatByYearyueday(year,yue,day);
	var today = formatByYearyueday(nowYear,nowMonth,nowDay);
	if(nowDate<=today){
		$("#appointment_btn").addClass("layui-btn-disabled");
	}else{
		$("#appointment_btn").removeClass("layui-btn-disabled");
	}
	
	console.log(nowDate);
	$("#now_date").html("<h4>"+nowDate+" "+getWeek(date.getDay())+"</h4>");
	$("#hidden_date").val(nowDate);
	$.post(ctx + '/book/appointmentCalendar/getDate',{appointment_date:nowDate},function(data){
		var app_state = 1;
		for (var i=0; i < data.length; i++) {
			var temp = data[i];
			if(temp.date_flag==1){
				$("#am_count").val(temp.appointment_count);
				app_state = temp.appointment_state;
				$("#am_id").val(temp.id)
			}else if(temp.date_flag==2){
				$("#pm_count").val(temp.appointment_count)
				app_state = temp.appointment_state;
				$("#pm_id").val(temp.id)
			}
		}
		layui.use(['form'], function(){
		    var form = layui.form;
			if (app_state == 1) {
				$('[name="appointment_state"]').prop('checked', true);
			} else {
				$('[name="appointment_state"]').prop('checked', false);
			}
